...
//        WHERE some_column=some_value;
    ...
    ExecuteWithFunction: function(type, con_db, cmd_sql, success_function) {
        var query = con_db.query(cmd_sql);
        query.exec( function( err, res ){
            if( err ){
                console.log(type + ' error');
    ...
    // execute with async callback function
    ExecuteCallback: function(type, con_db, cmd_sql, callback) {
        var query = con_db.query(cmd_sql);
        query.exec( function( err, res ){
            if( err ){
                console.log(type +' error: '+ err);
            }else{
                callback(null, type + ' command(s) completed successfully');
            }
        });
    },
    // execute with async callback function
